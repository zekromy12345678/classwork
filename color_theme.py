...
def update_color():
    ...
    #make the color box smaller than the original due to ipadx and ipady on the original color box
    ...
    #Display the tuple and hex values for the given color
...
def store_color():
    ...
    #Get the current value of each slider and append
    ...
    #Keep reference of current color
    stored_red = red_slider.get()
    stored_green = green_slider.get()
    stored_blue = blue_slider.get()

    #Create new widgets for the stored colors
    recall_button = tkinter.Button(output_frame, text="Recall Button", command=lambda:set_color(stored_red, stored_green, stored_blue))
    ...
    #Update the dictionary to hold stored colors
...
for i in range(6):
    ...
    recall_button = tkinter.Button(output_frame, text="Recall Button", state=DISABLED)
    ...
    recall_button.grid(row=i, column=1)
    new_color_tuple.grid(row=i, column=2)
    new_color_hex.grid(row=i, column=3)
    new_color_black_box.grid(row=i, column=4)
